...
sig Violation{
	...
	(no disj v1,v2: Violation | extractData[v1] = extractData[v2]) and	//No duplicated violations
	(some n:Notification | n.viol = this)	//all violations are notified (useless to consider unnoticed ones)
}
...
sig Notification{
	...
	(modified = True => (some n:Notification | this!=n and modify[n,this]) else (data = extractData[viol])) and	//A5,A6: no self-generated modified tasks, new notifications are generated by a violation
	fullData[this] and
	(one c: Citizen | this in c.sent)
}
...
pred store[n:Notification]{
	(n in SafeStreets.storedNotifications and all c:Citizen | n in c.sent=>n in c.acked) and
	(not n in SafeStreets.storedNotifications and all c:Citizen | n in c.sent=>n in c.nacked)
}
